...
| Invoice line delete → 405 `Method "DELETE" not allowed` (code) | **Fixed** | No | `http_method_names` missing `delete` on purchase/sales/quotation viewsets; hard invoice delete still blocked; draft-only + recalculation; tests in `test_invoice_line_delete.py` | Deploy + smoke (2026-07-10) |
| Purchase line total pieces×price instead of kg×price (code) | **Fixed** | No | Frontend purchase lines forced to `priceType=kg` (`السعر لكل كيلو`); backend recomputes `kg × unit_price` server-side; KG tests added | Deploy + smoke (2026-07-10) |
| Backdated invoice approval blocked (code) | **Fixed** | No | Stored-reason fallback in mixin; approve serializers accept `backdate_reason`; frontend local-date fix + reason on approve | Deploy + smoke (2026-07-10) |
| Mixed cashbox/bank selector (code) | **Fixed** | No | Separate cashbox/bank dropdowns per payment method with balances; `?account_type=&is_active=` filters; account-type validation at approval | Deploy + smoke (2026-07-10) |
| Payments screen crash on live movements (code) | **Fixed locally** | No | API returned `customer_collection` / `bank_transfer`; UI badges expected `collection` / `bank`, causing `undefined is not iterable` during render | Deploy + Payments screen smoke |
| Payment cancellation treasury reversal (code) | **Fixed locally** | No | Cancel now posts opposite `MoneyMovement` for `payment_movement` references; restores selected cashbox/bank balance atomically | Deploy + AED 10 cancel smoke |
| Root domain restores tenant dashboard (code) | **Fixed locally** | No | Session restore now validates user/host compatibility; JWTs are host-namespaced; backend rejects `auth_host_mismatch` on login/refresh/me/protected endpoints | Deploy + root/tenant cross-session smoke |
