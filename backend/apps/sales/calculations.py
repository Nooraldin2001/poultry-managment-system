"""Pure sales calculation helpers (no DB access)."""
...
def line_subtotal(*, price_type, unit_price, quantity_cartons=ZERO,
    ...
    """Line subtotal based on price_type. Free lines always return 0."""
    if is_free:
        return ZERO
...
def gross_profit(*, revenue, fifo_cost) -> Decimal:
    return (_d(revenue) - _d(fifo_cost)).quantize(MONEY_Q)
